...
import {changeTodoName, setActiveTodo} from "../../../redux/slices/todos";
...
export const TodoItem: FC<TodoItemProps> = ({name, countOfPomodoro, id, isActive}) => {
  ...
  const changeTaskNameCallback = () => {
    dispatch(changeTodoName({name: taskName, id: id}))
    setShowInput(false)
  }
  ...
  return (<li className={cn(s.todoItem, isActive ? s.todoItemActive : '' )} onClick={() => dispatch(setActiveTodo(id))}>
